...
2. char型要素数4の配列を用意し、各値のアドレスと値を表示させてみよ。
3. 和の関数シグマを定義せよ。
4. 積の関数パイを定義せよ。
5. 数値配列に変更加える。
6. 数値配列の値を変更した結果を別の配列に入れる。
7. 整数の割り算の商と余りを返す関数
8. 文字列を走査し指定した文字の位置を返す関数
9. 配列から指定した条件に一致したものと、一致しなかったものを返す
10. scanfを利用してみる。整数値の入力
11. scanfを利用してみる。簡易的なcsv入力
...
** IPv4のブロードキャストアドレス
   ...
   4. IPv4アドレス,サブネットマスクは4バイト(32ビット)の情報なので、ungiend int 型に収まるはずである。
      ...
      この考えを元に、共用体を用いてこれまでのように、IPv4の情報をunsigned char 型, 要素4の配列として扱いながらも、
      int型の整数値としても扱えるようにせよ。

   5. 練習としてプログラムに思いつく修正を加えてみよ。
      1. ネットワークアドレスを算出する関数を作成せよ。
      2. IPv4アドレスの設定を 192.168.10.3の様な形式で指定できるようにせよ。
      3. ブロードキャストアドレスを192.168.10.3/25の様な形式で指定できるようにせよ。
