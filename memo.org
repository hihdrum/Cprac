...
** 問題
   ...
   1. 1 + 2 の計算結果を終了ステータスとして返すプログラムを作成しなさい。(prac02.c)
   2. 1から10までの整数の和を終了ステータスとして返すプログラムを作成しなさい。(returnWa1to10.c)
      (難しく考えず、電卓を利用するように考えてみよ。)
   3. gccのオプション-Wallについてマニュアルを調べなさい。
      警告を出力してくれるので積極的な利用をおすすめします。
   4. 画面に以下のような文字列を出力するプログラムhello2を作成しなさい。
      #+begin_example
      hello
      hello
      #+end_example
   5. 画面に以下のような文字列を出力するプログラムhello3を作成しなさい。
      #+begin_src
      hello
       hello
        hello
      #+end_src
   6. 画面に以下のような文字列を出力するプログラムtreeを作成しなさい。
...
*** 問題
**** 常に失敗(0)を返す関数falseを作成し実際に使用してみよ。(false.c)

#+begin_src C :tangle false.c
  int main(void)
  {
    return 1;
  }
...
**** float型の引数を受け取り、その値の2乗を返す関数square_floatを作成し、実際に使用してみよ。
...
**** float型の引数を受け取り、その値の3乗を返す関数cube_floatを作成し、実際に使用してみよ。(cube.c)
...
**** float型の引数を受け取り、その値の4乗を返す関数quad_floatを作成し、実際に使用してみよ。
...
**** 2つの整数の和を計算する関数addを作成し実際に使用してみよ。(add.c)
...
**** Hello, world!と画面に出力する関数printHelloを作成しなさい。
