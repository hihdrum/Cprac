...
*** 問題
1. 常に失敗(0)を返す関数falseを作成し実際に使用してみよ。
...
4. float型の引数を受け取り、その値の4乗を返す関数quad_floatを作成し、実際に使用してみよ。
   作成にあたり、square_floatを利用せよ。
6. 2つの整数の和を計算するaddを作成し実際に使用してみよ。
...
*** 問題
1. 条件判定のif (xxxx) { ... } のxxxの部分は式なので1+2などの様に値を持つ
   10 == 10, 10 == 11 が実際にどの様な値になるかを確認してみなさい。
   難しく考えずに、結果とテキトーに予想しprintfしてみよ。
   出力結果から、すくなくともCが真と偽をどのような値として解釈しているといえるだろうか。

2. 1の結果を踏まえて、論理積（&&), 論理和(||)の結果を確認するサンプルプログラムを作成してみよ。
...
switch文はbreakを記述しない場合、上から下に処理が行われていいきます。
...
関数型のコードの利点は数学的な記述に近いかたちでコードを記述できることです。

関数型言語では、末尾再帰の形で記述した再帰処理がgotoになるそうです。
そのため、再帰呼び出しによってスタックを使い果たすことはありません。
しかし、C言語の場合は再帰呼出しの際にスタックを使用しますので、
再帰呼び出しが深くなるとプログラムが異常終了します。

以下は再帰を用いて階乗を計算するプログラムである。
...
4. FizzBuzz問題
   1. void printFizzBuzz(int)の実装

      3の倍数の場合 : Fizz
      5の倍数の場合 : Buzz
     15の倍数お場合 : FizzBuzz
      それ以外の場合 : 数値そのまま
     を画面に出力する関数void printFuzzBuzz(int)を作成せよ。

   2. 以下のmain関数追記しプログラムを実行してみよ。
...
　　　　　　* 
  ...
  6. 2点間のデータの平均値で補完
	...
	※この補完方法なら任意の値に対応できる関数のモデルが得られるな。...微妙
...
int main(void)
{
  ...
  printf("pxの値:%p, pxのメモリアドレス: %p, pxが指すメモリアドレスの値:%d\n",px, &px, *px);
...
名前, メモリに保持する値,  　　メモリアドレス
  x,              10,   0x7ffe40e0ce9c
 px,  0x7ffe40e0ce9c,   0x7ffe40e0cea0

xの値を取得するには、識別子xでアクセスし、メモリの保持する値10を得る。
pxの値を取得するには、識別子pxでアクセスし、メモリの保持する値0x7ffe40e0ce9cを得る。

  リファレンス  &x =   0x7ffe40e0ce9c  = px
デリファレンス   x = *(0x7ffe40e0ce9c) = *px
...
** 問題
1. char型要素数3の配列を用意し、各値のアドレスと値を表示させてみよ。
2. 
