...
5. helloworld.cのプリプロセス出力結果を確認してみよ。
   printfのプロトタイプ宣言はあるだろうか。

6. helloworld.cから#include <stdio.h>を削除して、
   プリプロセス出力結果を確認してみよ。
...
switch文はbreakを記述しない場合、上から下に処理が行われていく。

以下は、評価A〜Dから合否を判定するプログラムである。
実際に記述し実行してみよ。
...
　　　　　　　出たり止まったりする。一度警告状態となったら、ある程度水量が下がった後に警告を解除する様にしてほしい。
   ...
   4. 0からnまでの整数の和を求める関数を再帰呼び出しを使い実装せよ。
      具体的にすると
